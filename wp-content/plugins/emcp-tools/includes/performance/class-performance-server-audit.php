 ...
 * @since 3.0.0
 */
class EMCP_Tools_Performance_Server_Audit {
	...
	public function evaluate_php_version( string $version ): array {
		if ( version_compare( $version, '8.2', '>=' ) ) {
			return EMCP_Tools_Performance_Finding::make( 'php_version', 'server', 'PHP version', 'pass', $version, sprintf( 'PHP %s is current and supported.', $version ) );
		}
		if ( version_compare( $version, '8.0', '>=' ) ) {
			return EMCP_Tools_Performance_Finding::make( 'php_version', 'server', 'PHP version', 'warning', $version, sprintf( 'PHP %s is nearing end of life.', $version ), 'Upgrade to PHP 8.2 or newer for better performance and security support.' );
		}
		return EMCP_Tools_Performance_Finding::make( 'php_version', 'server', 'PHP version', 'critical', $version, sprintf( 'PHP %s is end-of-life and unsupported.', $version ), 'Upgrade to PHP 8.2+ immediately; old PHP is slow and a security risk.' );
	}

	public function evaluate_memory_limit( string $limit ): array {
		$bytes = $this->to_bytes( $limit );
		if ( $bytes < 0 ) { // -1 = unlimited.
			return EMCP_Tools_Performance_Finding::make( 'memory_limit', 'server', 'PHP memory limit', 'pass', $limit, 'PHP memory is unlimited.' );
		}
		if ( $bytes >= self::MIN_MEMORY_BYTES ) {
			return EMCP_Tools_Performance_Finding::make( 'memory_limit', 'server', 'PHP memory limit', 'pass', $limit, sprintf( 'PHP memory limit is %s.', $limit ) );
		}
		return EMCP_Tools_Performance_Finding::make( 'memory_limit', 'server', 'PHP memory limit', 'warning', $limit, sprintf( 'PHP memory limit is only %s.', $limit ), 'Raise memory_limit (and WP_MEMORY_LIMIT) to at least 128M to avoid out-of-memory errors under load.' );
	}
	...
	public function evaluate_object_cache( bool $persistent ): array {
		return $persistent
			? EMCP_Tools_Performance_Finding::make( 'object_cache', 'server', 'Persistent object cache', 'pass', true, 'A persistent object cache is active.' )
			: EMCP_Tools_Performance_Finding::make( 'object_cache', 'server', 'Persistent object cache', 'warning', false, 'No persistent object cache detected.', 'Add Redis or Memcached with a drop-in (e.g. redis-cache) to cache DB queries across requests.' );
	}

	public function evaluate_image_lib( bool $imagick, bool $gd ): array {
		if ( $imagick || $gd ) {
			return EMCP_Tools_Performance_Finding::make( 'image_lib', 'server', 'Image library', 'pass', $imagick ? 'imagick' : 'gd', 'An image processing library is available.' );
		}
		return EMCP_Tools_Performance_Finding::make( 'image_lib', 'server', 'Image library', 'warning', 'none', 'No image library (Imagick/GD) detected.', 'Install Imagick or GD so WordPress can generate optimized image sizes.' );
	}
	...
	public function evaluate_plugin_count( int $count ): array {
		if ( $count > self::PLUGIN_WARN_COUNT ) {
			return EMCP_Tools_Performance_Finding::make( 'plugin_count', 'config', 'Active plugins', 'warning', $count, sprintf( '%d active plugins.', $count ), 'A large plugin count compounds per-request overhead. Audit for unused or overlapping plugins.' );
		}
		return EMCP_Tools_Performance_Finding::make( 'plugin_count', 'config', 'Active plugins', 'info', $count, sprintf( '%d active plugins.', $count ) );
	}

	public function evaluate_revisions( int $count ): array {
		if ( $count > self::REVISIONS_WARN_COUNT ) {
			return EMCP_Tools_Performance_Finding::make( 'post_revisions', 'database', 'Post revisions', 'warning', $count, sprintf( '%d post revisions stored.', $count ), 'Cap revisions with define( "WP_POST_REVISIONS", 5 ) and clean old ones to shrink the posts table.' );
		}
		return EMCP_Tools_Performance_Finding::make( 'post_revisions', 'database', 'Post revisions', 'info', $count, sprintf( '%d post revisions stored.', $count ) );
	}

	public function evaluate_cron_backlog( int $overdue ): array {
		if ( $overdue > 0 ) {
			return EMCP_Tools_Performance_Finding::make( 'cron_backlog', 'config', 'WP-Cron backlog', 'warning', $overdue, sprintf( '%d overdue cron events.', $overdue ), 'A backlog means cron is not firing (low traffic, or DISABLE_WP_CRON without a real cron job). Add a server cron hitting wp-cron.php.' );
		}
		return EMCP_Tools_Performance_Finding::make( 'cron_backlog', 'config', 'WP-Cron backlog', 'pass', 0, 'No overdue cron events.' );
	}
	...
	public function evaluate_autoload_size( int $bytes, array $top_options ): array {
		...
		return EMCP_Tools_Performance_Finding::make( 'autoload_size', 'database', 'Autoloaded options', 'pass', $value, sprintf( 'Autoloaded options total %s.', $human ) );
	}

	public function evaluate_database_size( int $bytes, array $top_tables ): array {
		return EMCP_Tools_Performance_Finding::make( 'database_size', 'database', 'Database size', 'info', array( 'bytes' => $bytes, 'top_tables' => $top_tables ), sprintf( 'Database is %s; %d largest tables listed.', $this->human_bytes( $bytes ), count( $top_tables ) ) );
	}
